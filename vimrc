...
"------------------------------------
" vim-fugitive: ブランチ差分レビュー
"------------------------------------
" 使い方:
"   :DiffFiles                引数省略時はHEAD（未コミットの差分）が対象
"   :DiffFiles main           対象ブランチ/コミットとの差分ファイル一覧をquickfixに表示
"   :DiffFiles main HEAD~3    2点間の比較も可
"   ]q / [q                  次/前のファイルへ移動し、自動でGdiffsplitを開く
"   <Leader>D                :DiffFiles の入力を促す
"   <Leader>Q                レビュー終了（quickfixを閉じてrefをクリア）
"
" 差分の左側（git blob側）は読み取り専用のfugitive://バッファなのでLSPは効かない。
" gd/gr等でコードを追うときは右側の実ファイル側にカーソルを置く。
let g:diff_review_ref = ''
" レビュー対象の実ファイルを表示する「メインウィンドウ」と、その都度
" Gdiffsplitで開く「比較用ウィンドウ（git blob側）」をIDで追跡する。
" これがないと、比較用ウィンドウにカーソルがある状態で]qを押した時に
" 誤ったウィンドウでcnextが実行され、分割がどんどん積み重なってしまう。
let s:diff_review_main_winid = 0
let s:diff_review_companion_winid = 0

function! s:git_root() abort
  let root = system('git rev-parse --show-toplevel 2>/dev/null')
  if v:shell_error
    echoerr 'DiffFiles: gitリポジトリではありません'
    return ''
  endif
  return substitute(root, '\n\+$', '', '')
endfunction

function! s:diff_files(args) abort
  let root = s:git_root()
  if empty(root)
    return
  endif
  ...
  let files = systemlist('git -C ' . shellescape(root) . ' diff --name-only ' . ref . ' --')
  if v:shell_error
    echoerr 'DiffFiles: git diffに失敗しました: ' . ref
    return
  endif
  if empty(files)
    echomsg 'DiffFiles: ' . ref . ' との差分はありません'
    return
  endif

  let g:diff_review_ref = ref
  let s:diff_review_main_winid = win_getid()
  let s:diff_review_companion_winid = 0
  let qf = map(files, '{"filename": root . "/" . v:val, "text": v:val}')
  call setqflist(qf, ' ')
  call setqflist([], 'a', {'title': 'DiffFiles: ' . ref})
  copen
endfunction

command! -nargs=* DiffFiles call s:diff_files(<q-args>)

" メインウィンドウへ戻る。閉じられていたら諦めてエラーを出す
" （ウィンドウを見失った状態で cnext/Gdiffsplit を続けると分割が暴走するため）。
function! s:diff_review_goto_main() abort
  if s:diff_review_main_winid isnot 0 && win_gotoid(s:diff_review_main_winid)
    return 1
  endif
  echoerr 'DiffFiles: レビュー用ウィンドウが見つかりません。:DiffFiles をやり直してください'
  return 0
endfunction

function! s:diff_review_open() abort
  if empty(g:diff_review_ref) || !s:diff_review_goto_main() || &buftype !=# ''
    return
  endif
  " 前回の比較用ウィンドウを閉じる。diffペアが1枚になるとVimが自動で
  " diffoffするので、メインウィンドウ側は特に後始末しなくてよい。
  if s:diff_review_companion_winid isnot 0 && win_id2win(s:diff_review_companion_winid) > 0
    execute win_id2win(s:diff_review_companion_winid) . 'close'
    call s:diff_review_goto_main()
  endif
  execute 'Gdiffsplit' g:diff_review_ref
  let s:diff_review_companion_winid = win_getid()
  call s:diff_review_goto_main()
endfunction

" cnext/cprev/cc を実行する。エラーなら1、失敗（リスト端）なら0を返す。
" DiffFilesレビュー中でなければ（g:diff_review_ref が空なら）、これらは
" 素のquickfix移動として動く＝:grepや:make等の通常のquickfix利用を壊さない。
function! s:diff_review_try_move(cmd) abort
  try
    execute a:cmd
    return 1
  catch /^Vim\%((\a\+)\)\=:E55[03]/
    echo 'DiffFiles: ' . (a:cmd ==# 'cnext' ? 'これが最後のファイルです' : 'これが最初のファイルです')
    return 0
  endtry
endfunction

function! s:diff_review_jump(cmd) abort
  if empty(g:diff_review_ref)
    call s:diff_review_try_move(a:cmd)
    return
  endif
  if !s:diff_review_goto_main() || !s:diff_review_try_move(a:cmd)
    return
  endif
  let s:diff_review_main_winid = win_getid()
  call s:diff_review_open()
endfunction

function! s:diff_review_qf_enter() abort
  let idx = line('.')
  if empty(g:diff_review_ref)
    execute 'cc' idx
    return
  endif
  if !s:diff_review_goto_main()
    return
  endif
  execute 'cc' idx
  let s:diff_review_main_winid = win_getid()
...
function! s:diff_review_end() abort
  let g:diff_review_ref = ''
  let s:diff_review_main_winid = 0
  let s:diff_review_companion_winid = 0
  cclose
...
nnoremap <silent> ]q :call <SID>diff_review_jump('cnext')<CR>
nnoremap <silent> [q :call <SID>diff_review_jump('cprev')<CR>
nnoremap <Leader>D :DiffFiles<Space>
nnoremap <silent> <Leader>Q :call <SID>diff_review_end()<CR>

augroup diff_review_qf
  autocmd!
  autocmd FileType qf nnoremap <buffer><silent> <CR> :call <SID>diff_review_qf_enter()<CR>
augroup END
